class DarkMode {
    static async Menu() {
      ...
      if (typeof DarkMode_menu === "undefined") {
        ...
        DarkMode_menu.style.backgroundColor = "#121212"; // Dark background
        DarkMode_menu.style.position = "absolute";
        DarkMode_menu.innerHTML = `
                  <h1 style="color: #ffffff; text-align: center;">Dark Mode</h1>
    ...
    static async toggleToWhitelist(domain) {
      ...
      if (!toggleList.darken.includes(domain)) {
        ...
        toggleList.darken = toggleList.darken.filter((item) => item !== domain);
      }
  ...
  Page.register({
    ...
    Info: "Dark Mode extension for BayMax.",
    Description:
      "This extension can toggle a dark mode on pages viewed via BayMax.",
    page: "DarkMode",
    name: "DarkMode",
  });
